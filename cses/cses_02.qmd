...
```{r}
cses_isr %>%
  dplyr::select(compromise_betrayal, dont_care_ppl, trustworthy, main_problem, decision_by_ppl, care_tycoons) %>%
...
```{r}
cfa_m <- '
one_factor = ~ compromise_betrayal + dont_care_ppl + trustworthy + main_problem + decision_by_ppl + care_tycoons '
...
popular_sovereignty = ~ decision_by_ppl
manichean = ~ compromise_betrayal '
...
cses_isr <- cses_isr %>%
  mutate(anti_elitism = (((dont_care_ppl + trustworthy + main_problem + care_tycoons) / 4-1) / 4),
         popular_sovereignty = ((decision_by_ppl - 1) / 4),
         manichean = ((compromise_betrayal - 1) / 4)) %>%
