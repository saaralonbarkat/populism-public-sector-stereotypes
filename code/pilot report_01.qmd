...
pilot %>%
  ...
  filter(political_block %in% 1:3) %>% 
  ggplot(aes(x = affective_polarization, y = populist_attitudes_index_4dim)) +
  geom_point(alpha=0.2) +
...
pilot %>%
  ...
  facet_wrap(vars(political_block %>% labelled::to_factor()))+
  theme_minimal()
```
...
pilot %>%
  drop_na(affective_polarization) %>% 
  filter(political_block %in% 1:4) %>% 
  ggplot(aes(x = right_left_ideology, y = affective_polarization,color=political_block%>% labelled::to_factor())) +
  geom_jitter(alpha=0.5) +
  geom_smooth(method="lm")+
  stat_summary(fun = mean, geom = "point", shape = 21, size = 3, fill = "red", color = "black") +
  labs(
    x = "Right left ideology (7=extreme left)",
    y = "Affective polarization\n(abs(affect_right-affect_left))",
    title = "Populist Attitudes by affect_left",
...
model_1 <- lm(populist_attitudes_index_4dim ~ female +
                ...
                external_efficecy+
                subjective_social_economic_status+
                affective_polarization,pilot %>% labelled::remove_labels() %>% drop_na(right_left_ideology),na.rm=T)
...
```{r}
coded_responses <- read_csv("data/manual coding all civil servants 6.6.25 - automatically _coded_output 9.6.25.csv")
...
responses %>% 
  ...
  theme_blank()+
  labs(title = "Ability - dimension distribution")
...
responses %>% 
  ...
  facet_wrap(vars(entity))+
  theme_blank()+
  labs(title = "Morality - dimension distribution")
...
responses %>% 
  ...
  facet_wrap(vars(entity))+
  theme_blank()+
  labs(title = "Assertiveness - dimension distribution")
...
Civil servants
```{r}
tmod1 <- lm(response_direction~populist_attitudes_index_4dim,data=responses %>% filter(entity=="civil servants") %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
                ...
                external_efficecy+
                subjective_social_economic_status+
                  public_sector_employee_current)
...
```{r}
tmod1 <- lm(response_direction~populist_attitudes_index_4dim,data=responses %>% filter(entity=="junior civil servants") %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
```{r}
tmod1 <- lm(response_direction~populist_attitudes_index_4dim,data=responses %>% filter(entity=="senior civil servants") %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
responses %>% 
  ...
  ggplot(aes(x=populist_attitudes_index_4dim,y=morality))+
  geom_point()+
...
responses %>% 
  filter(entity=="civil servants") %>% 
  ggplot(aes(x=populist_attitudes_index_4dim,y=ability))+
  geom_point()+
  geom_smooth(method = "glm", method.args = list(family = "binomial"))+
  theme_blank()
...
responses %>% 
  filter(entity=="civil servants") %>% 
  ggplot(aes(x=populist_attitudes_index_4dim,y=assertiveness))+
  geom_point()+
  geom_smooth(method = "glm", method.args = list(family = "binomial"))+
  theme_blank()
...
responses %>% 
  filter(entity=="civil servants") %>% 
  ggplot(aes(x=populist_attitudes_index_4dim,y=status.x))+
  geom_point()+
  geom_smooth(method = "glm", method.args = list(family = "binomial"))+
  theme_blank()
...
```{r}
tmod1 <- glm(morality~populist_attitudes_index_4dim,family="binomial" ,data=responses %>% filter(entity=="civil servants") %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
sjPlot::tab_model(tmod1,tmod2,
                  ...
                  tmod5,tmod6,
                  collapse.ci = T,
                  title = "Civil servants - frequency of dimensions")
```
...
responses %>% 
  filter(entity=="civil servants") %>% 
  ggplot(aes(x=anti_elitism,y=morality))+
  geom_point()+
  geom_smooth(method = "glm", method.args = list(family = "binomial"))+
  theme_blank()
...
responses %>% 
  filter(entity=="civil servants") %>% 
  ggplot(aes(x=anti_elitism,y=ability))+
  geom_point()+
  geom_smooth(method = "glm", method.args = list(family = "binomial"))+
  theme_blank()
...
responses %>% 
  filter(entity=="civil servants") %>% 
  ggplot(aes(x=anti_elitism,y=assertiveness))+
  geom_point()+
  geom_smooth(method = "glm", method.args = list(family = "binomial"))+
  theme_blank()
...
```{r}
tmod1 <- lm(response_direction~populist_attitudes_index_4dim,data=responses %>% filter(entity=="civil servants",morality==1) %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
sjPlot::tab_model(tmod1,tmod2,
                  ...
                  show.std = T,
                  collapse.ci = T,
                  title = "Civil servants - direction of morality")
```
...
```{r}
tmod1 <- glm(morality~populist_attitudes_index_4dim,family="binomial" ,data=responses %>% filter(entity=="junior civil servants") %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
```{r}
tmod1 <- lm(response_direction~populist_attitudes_index_4dim,data=responses %>% filter(entity=="junior civil servants",morality==1) %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
```{r}
tmod1 <- lm(response_direction~populist_attitudes_index_4dim,data=responses %>% filter(entity=="junior civil servants",ability==1) %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
```{r}
tmod1 <- lm(response_direction~populist_attitudes_index_4dim,data=responses %>% filter(entity=="junior civil servants",assertiveness==1) %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
```{r}
tmod1 <- glm(morality~populist_attitudes_index_4dim,family="binomial",data=responses %>% filter(entity=="senior civil servants") %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
```{r}
tmod1 <- lm(response_direction~populist_attitudes_index_4dim,data=responses %>% filter(entity=="senior civil servants",morality==1) %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
```{r}
tmod1 <- lm(response_direction~populist_attitudes_index_4dim,data=responses %>% filter(entity=="senior civil servants",ability==1) %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
```{r}
tmod1 <- lm(response_direction~populist_attitudes_index_4dim,data=responses %>% filter(entity=="senior civil servants",assertiveness==1) %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
```{r}
tmod1 <- lme4::glmer(morality~populist_attitudes_index_4dim + entity + (1|pid.x),family="binomial",data=responses  %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
...
```{r}
tmod1 <- lme4::lmer(response_direction~populist_attitudes_index_4dim + entity + (1|pid.x),data=responses  %>% labelled::remove_labels())
tmod2 <- tmod1 %>% update(.~.+
                            female +
                age_group+
                income+
                ...
                subjective_social_economic_status+
                  public_sector_employee_current)
tmod3 <- tmod2 %>% update(.~.+populist_attitudes_index_4dim*entity)
...
```{r}
responses %>% 
  ggplot(aes(x=affective_polarization,y=response_direction))+
...
```{r}
responses %>% 
  ggplot(aes(x=factor(right_left_ideology),y=response_direction))+
  stat_summary(fun = mean, geom = "point", shape = 21, size = 3, fill = "red", color = "black") +
      stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2) +
  facet_wrap(vars(entity))+
  theme_blank()
