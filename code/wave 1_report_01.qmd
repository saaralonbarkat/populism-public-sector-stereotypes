...
sjPlot::plot_model(model_4,type = "pred",terms = c("populist_attitudes_index_4dim", "pro_netanyahu_coalition"))+
  labs(title="Trust in Civil Service",
  x="Populist attitudes index",
  color="Partisanship")+
  theme_classic()
```
...
```{r}
sjPlot::plot_model(model_4,type = "pred",terms = c("anti_elitism", "pro_netanyahu_coalition"))
```
...
```{r}
wave1 %>% labelled::to_factor() |> 
  dplyr::select(conspiracy_deep_state,conspiracy_vaccine,coalition_party_vote_intention) %>% 
  gtsummary::tbl_summary(by=coalition_party_vote_intention)
```
