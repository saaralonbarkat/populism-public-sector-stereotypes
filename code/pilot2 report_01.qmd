...
```{r}
pilot2 %>%
      dplyr::select(populist_attitudes_index_3dim,
      anti_elitism,
      homogeneity,
      popular_sovereignty,
      manichean,
...
```{r}
model_1 <- lm(trust_civil_service ~ populist_attitudes_index_4dim + coalition_party_vote_intention + right_left_ideology ,pilot2 %>% labelled::remove_labels(),na.rm=T)
...
pilot2 %>%
  ...
  labelled::remove_labels() %>%
  dplyr::select(
    populist_attitudes_index_3dim,populist_attitudes_index_4dim,
        popular_sovereignty:manichean,
    conspiracy_deep_state_n,
    conspiracy_vaccine_n,
    trust_knesset:trust_media
  ) %>%
  sjPlot::tab_corr(triangle = "lower", title = "Netanyahu coalition")
```
...
pilot2 %>%
  ...
  sjPlot::tab_corr(triangle = "lower", title = "Anti Netanyahu coalition")
```
...
```{r}
model_1 <- lm(conspiracy_deep_state_n ~ populist_attitudes_index_4dim + coalition_party_vote_intention + right_left_ideology ,pilot2 %>% labelled::remove_labels(),na.rm=T)
...
```{r}
responses$populist_attitudes_index_4dim

responses %>% 
  filter(morality==1) %>% 
  ggplot(aes(x=right_left_ideology,y=populist_attitudes_index_4dim))+
  geom_text(aes(label = imagined_entity,colour = response_direction),size=2,position = position_jitter())+
  geom_smooth(method="lm")+
  theme_classic()
