...
```{r}
pilot2 |> 
  ggplot(aes(x=popular_sovereignty,y=trust_civil_service,color=pro_netanyahu_coalition)) +
geom_jitter()  +
...
```{r}
model_1 <- lm(trust_civil_service ~ populist_attitudes_index_4dim + pro_netanyahu_coalition + right_left_ideology ,pilot2 %>% labelled::remove_labels(),na.rm=T)
...
```{r}
model_1 <- lm(trust_civil_service ~ anti_elitism + pro_netanyahu_coalition + right_left_ideology ,pilot2 %>% labelled::remove_labels(),na.rm=T)
...
```{r}
model_1 <- lm(trust_police ~ populist_attitudes_index_4dim + pro_netanyahu_coalition + right_left_ideology ,pilot2 %>% labelled::remove_labels(),na.rm=T)
...
```{r}
model_1 <- lm(trust_media ~ populist_attitudes_index_4dim + pro_netanyahu_coalition + right_left_ideology ,pilot2 %>% labelled::remove_labels(),na.rm=T)
...
```{r}
model_1 <- lm(conspiracy_deep_state_n ~ populist_attitudes_index_4dim + pro_netanyahu_coalition + right_left_ideology ,pilot2 %>% labelled::remove_labels(),na.rm=T)
...
```{r}
model_1 <- lm(conspiracy_vaccine_n ~ populist_attitudes_index_4dim + pro_netanyahu_coalition + right_left_ideology ,pilot2 %>% labelled::remove_labels(),na.rm=T)
...
responses_pilot2 |> 
  group_by(pid.x) |> 
  summarise(n=n()) |>
  summarise(mean_n=mean(n),median_n=median(n),max_n=max(n),min_n=min(n)) 
...
```{r}
#icc calculation for response_direction for pid.x


tmod1<-aov(response_direction~as.factor(pid.x),data=responses_pilot2)
   multilevel::ICC1(tmod1)
...
responses_pilot2 %>% 
  mutate(response_direction = as.numeric(response_direction)) %>% 
  dplyr::select(assertiveness:other) %>% 
  gtsummary::tbl_summary(by=sociability)
...
responses_pilot2 %>% 
  mutate(response_direction = as.numeric(response_direction)) %>% 
  dplyr::select(assertiveness:other) %>% 
  gtsummary::tbl_summary(by=morality)
...
responses_pilot2 %>% 
  mutate(response_direction = as.numeric(response_direction)) %>% 
  dplyr::select(assertiveness:other) %>% 
  gtsummary::tbl_summary(by=ability)
...
responses_pilot2 %>% 
  mutate(response_direction = as.numeric(response_direction)) %>% 
  dplyr::select(assertiveness:other) %>% 
  gtsummary::tbl_summary(by=assertiveness)
```


```{r}
#distribution of direction responses by label

responses_pilot2 %>% 
  drop_na(civil_servant_label) %>%
  ggplot(aes(x=response_direction))+
  geom_bar(position = "dodge")+
  labs(title="All responses by civil_servant_label")
...
responses_pilot2 %>% 
  drop_na(civil_servant_label) %>%
  ggplot(aes(x=civil_servant_label,y=response_direction))+
  #geom_jitter(alpha=0.2)+
  stat_summary(fun = mean, geom = "point", shape = 21, size = 3, fill = "red", color = "black") +
      stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2)+
  labs(title="All responses by civil_servant_label")+
...
responses_pilot2 %>% 
  drop_na(coalition_party_vote_intention) %>%
  ggplot(aes(x=response_direction,fill=pro_ne))+
  geom_bar(position = "dodge")+
...
responses_pilot2 %>% 
  drop_na(civil_servant_label) %>%
  #filter(coalition_party_vote_intention!="Other") %>%
#filter(civil_servant_label!="public service bureaucrat") %>% 
  ggplot(aes(x=populist_attitudes_index_4dim_minimum,y=response_direction,color=pro_netanyahu_coalition))+
  geom_jitter(alpha=0.2)+
  #facet_wrap(vars(civil_servant_label))+
  geom_smooth(method="lm")+
  theme_blank()
...
t1 %>% 
  ...
  #filter(coalition_party_vote_intention!="Other") %>%
#filter(civil_servant_label!="public service bureaucrat") %>% 
  ggplot(aes(x=populist_attitudes_index_4dim,y=response_direction,color=pro_netanyahu_coalition))+
  geom_jitter(alpha=0.2)+
  facet_wrap(vars(religiosity_label |> labelled::to_factor()))+
  geom_smooth(method="lm")+
  theme_blank()+
  labs(title="Pilot 2")
```
...
#filter(civil_servant_label!="public service bureaucrat") %>% 
  ggplot(aes(x=populist_attitudes_index_4dim,y=response_direction,color=pro_netanyahu_coalition))+
  geom_jitter(alpha=0.2)+
  facet_wrap(vars(income |> labelled::to_factor()))+
  geom_smooth(method="lm")+
  theme_blank()+
  labs(title="Pilot 2")
```
...
```{r}
responses_pilot2 %>% 
  filter(religiosity==4, populist_attitudes_index_4dim>=0.8) %>% 
  dplyr::select(pid.x,response_direction,response,popular_sovereignty:manichean,populist_attitudes_index_4dim,conspiracy_deep_state,coalition_party_vote_past,civil_servant_label,imagined_entity) |> view()
```
...
tmod2 <- lme4::lmer(
  response_direction ~ populist_attitudes_index_4dim*pro_netanyahu_coalition +
    (1 | pid.x),
  data = responses_pilot2 %>% labelled::remove_labels()
)
...
tmod2 <- tmod1 %>%
  update(. ~ . + populist_attitudes_index_4dim * pro_netanyahu_coalition)
tmod3 <- tmod2 %>%
  update(
    . ~ . +
      right_left_ideology +
...
tmod4 <- lm(response_direction ~ populist_attitudes_index_4dim +
      right_left_ideology +
      female +
      age_group +
      income +
      ...
      affective_polarization +
      religiosity,
  data = responses_pilot2 %>% labelled::remove_labels() |> filter(pro_netanyahu_coalition=="Netanyahu coalition"
))


sjPlot::tab_model(tmod1, tmod2, tmod3, tmod4,show.est = F, show.std = T, collapse.ci = T)
```
...
tmod4 <- lme4::lmer(response_direction ~ populist_attitudes_index_4dim +
    civil_servant_label +
      right_left_ideology +
      female +
      age_group +
...
```{r}
sjPlot::plot_model(tmod3,type = "pred",terms = c("populist_attitudes_index_4dim", "pro_netanyahu_coalition"))
```
...
tmod4 <- tmod3 %>%
  update(
    . ~ . +
      populist_attitudes_index_4dim*civil_servant_label
  )
sjPlot::tab_model(tmod3, tmod4, show.est = F, show.std = T, collapse.ci = T)
anova(tmod4,tmod3)  
```
...
p2 <- responses_pilot2 %>% 
  ...
  #filter(coalition_party_vote_intention!="Other") %>%
#filter(civil_servant_label!="public service bureaucrat") %>% 
  ggplot(aes(x=anti_elitism,y=response_direction,color=pro_netanyahu_coalition))+
  geom_jitter(alpha=0.2)+
  #facet_wrap(vars(civil_servant_label))+
  geom_smooth(method="lm")+
  theme_blank()
...
p3 <- responses_pilot2 %>% 
  ...
  #filter(coalition_party_vote_intention!="Other") %>%
#filter(civil_servant_label!="public service bureaucrat") %>% 
  ggplot(aes(x=popular_sovereignty,y=response_direction,color=pro_netanyahu_coalition))+
  geom_jitter(alpha=0.2)+
  #facet_wrap(vars(civil_servant_label))+
  geom_smooth(method="lm")+
  theme_blank()
...
p4 <- responses_pilot2 %>% 
  ...
  #filter(coalition_party_vote_intention!="Other") %>%
#filter(civil_servant_label!="public service bureaucrat") %>% 
  ggplot(aes(x=homogeneity,y=response_direction,color=pro_netanyahu_coalition))+
  geom_jitter(alpha=0.2)+
  #facet_wrap(vars(civil_servant_label))+
  geom_smooth(method="lm")+
  theme_blank()
...
p5 <- responses_pilot2 %>% 
  ...
  #filter(coalition_party_vote_intention!="Other") %>%
#filter(civil_servant_label!="public service bureaucrat") %>% 
  ggplot(aes(x=manichean,y=response_direction,color=pro_netanyahu_coalition))+
  geom_jitter(alpha=0.2)+
  #facet_wrap(vars(civil_servant_label))+
  geom_smooth(method="lm")+
  theme_blank()
...
tmod1 <- lme4::lmer(
  response_direction ~ populist_attitudes_index_4dim +
    ...
    civil_servant_label +
    (1 | pid.x),
  data = responses_pilot2 %>% labelled::remove_labels()  |> filter(morality==1)
)
tmod2 <- tmod1 %>%
  update(. ~ . + populist_attitudes_index_4dim * pro_netanyahu_coalition)
tmod3 <- tmod2 %>%
...
sjPlot::tab_model(tmod1, tmod2, tmod3, show.est = F, show.std = T, collapse.ci = T)
```
...
tmod1 <- lme4::lmer(
  response_direction ~ populist_attitudes_index_4dim +
    ...
    civil_servant_label +
    (1 | pid.x),
  data = responses_pilot2 %>% labelled::remove_labels()  |> filter(ability==1)
)
tmod2 <- tmod1 %>%
  update(. ~ . + populist_attitudes_index_4dim * pro_netanyahu_coalition)
tmod3 <- tmod2 %>%
...
tmod1 <- lme4::lmer(
  response_direction ~ populist_attitudes_index_4dim +
    ...
    civil_servant_label +
    (1 | pid.x),
  data = responses_pilot2 %>% labelled::remove_labels()  |> filter(assertiveness==1)
)
tmod2 <- tmod1 %>%
  update(. ~ . + populist_attitudes_index_4dim * pro_netanyahu_coalition)
tmod3 <- tmod2 %>%
...
tmod1 <- lme4::lmer(
  response_direction ~ populist_attitudes_index_4dim +
    ...
    civil_servant_label +
    (1 | pid.x),
  data = responses_pilot2 %>% labelled::remove_labels()  |> filter(sociability==1)
)
tmod2 <- tmod1 %>%
  update(. ~ . + populist_attitudes_index_4dim * pro_netanyahu_coalition)
tmod3 <- tmod2 %>%
...
responses_pilot2 %>% 
  ...
  #coalition_party_vote_intention=="Netanyahu coalition") %>%
#filter(civil_servant_label!="public service bureaucrat") %>% 
  ggplot(aes(x=conspiracy_deep_state_n,y=response_direction,color=factor(pro_netanyahu_coalition)))+
  geom_jitter()+
  geom_smooth(method="lm")+
  #facet_wrap(vars(civil_servant_label))+
 #   labs(title="Morality - Netanyahu coalition")+
...
responses_pilot2 %>% 
  ...
  #coalition_party_vote_intention=="Netanyahu coalition") %>%
#filter(civil_servant_label!="public service bureaucrat") %>% 
  ggplot(aes(x=trust_civil_service,y=response_direction,color=factor(pro_netanyahu_coalition)))+
  geom_jitter()+
  geom_smooth(method="lm")+
  #facet_wrap(vars(civil_servant_label))+
 #   labs(title="Morality - Netanyahu coalition")+
...
```{r}
fitmeasures(fit.cfa.model.1factor_ster, c('chisq','df','pvalue','cfi','tli', 'rmsea', 'rmsea.ci.upper','rmsea.ci.lower', 'srmr')) %>% 
  broom::tidy() %>% 
  pivot_wider(names_from = names,values_from = x) %>% 
  mutate(chisq_01 = str_c(chisq %>% round(3)," [","df=",df,", p=",pvalue %>% round(3),"]"),
         rmsea_01 = str_c(rmsea %>% round(3)," CI[", rmsea.ci.lower %>% round(3),",",rmsea.ci.upper %>% round(3),"]"),
         ...
         tli = tli %>% round(3),
         srmr = srmr %>% round(3),
         n = fit.cfa.model.1factor_ster@SampleStats@nobs) %>%
  dplyr::select('N' = n,
          'Chi-sq' = chisq_01,
         'CFI' = cfi,
         'TLI' = tli,
...
cfa.model.4factor_ster <- '
  ...
  assertiveness =~   q55_11_self_confident + q55_12_pressure_coping + q55_13_give_up + q55_14_leadership + q55_15_superiority
...
```{r}
parameterEstimates(fit.cfa.model.2factor_ster, standardized=TRUE) %>% 
  filter(op == "=~") %>% 
  mutate(Beta = str_c(std.all %>% round(3)," (",pvalue %>% round(3),")")) %>% 
    dplyr::select('Latent Factor'=lhs,  
           'Indicator' = rhs,
...
```{r}
fitmeasures(fit.cfa.model.2factor_ster, c('chisq','df','pvalue','cfi','tli', 'rmsea', 'rmsea.ci.upper','rmsea.ci.lower', 'srmr')) %>% 
  broom::tidy() %>% 
  pivot_wider(names_from = names,values_from = x) %>% 
  mutate(chisq_01 = str_c(chisq %>% round(3)," [","df=",df,", p=",pvalue %>% round(3),"]"),
         rmsea_01 = str_c(rmsea %>% round(3)," CI[", rmsea.ci.lower %>% round(3),",",rmsea.ci.upper %>% round(3),"]"),
         ...
         tli = tli %>% round(3),
         srmr = srmr %>% round(3),
         n = fit.cfa.model.2factor_ster@SampleStats@nobs) %>%
  dplyr::select('N' = n,
          'Chi-sq' = chisq_01,
         'CFI' = cfi,
         'TLI' = tli,
...
```{r}
#histogram of sociability_scale,  morality_scale, ability_scale,  assertiveness_scale

p1<- responses_pilot2 %>%
  ggplot(aes(x=sociability_scale))+
  geom_bar()+
  theme_blank()

p2 <- responses_pilot2 %>%
  ggplot(aes(x=morality_scale))+
  geom_bar()+
  theme_blank()

p3 <- responses_pilot2 %>%
  ggplot(aes(x=ability_scale))+
  geom_bar()+
  theme_blank()

p4 <- responses_pilot2 %>%
  ggplot(aes(x=assertiveness_scale))+
  geom_bar()+
  theme_blank()
...
p1<- responses_pilot2 %>%
  drop_na(pro_netanyahu_coalition) |> 
  ggplot(aes(x=pro_netanyahu_coalition, y=sociability_scale))+
  geom_boxplot()+
  theme_blank()+
  coord_flip()
...
p2<- responses_pilot2 %>%
  drop_na(pro_netanyahu_coalition) |> 
  ggplot(aes(x=pro_netanyahu_coalition, y=morality_scale))+
  geom_boxplot()+
  theme_blank()+
  coord_flip()
...
p3<- responses_pilot2 %>%
  drop_na(pro_netanyahu_coalition) |> 
  ggplot(aes(x=pro_netanyahu_coalition, y=ability_scale))+
  geom_boxplot()+
  theme_blank()+
  coord_flip()
...
p4<- responses_pilot2 %>%
  drop_na(pro_netanyahu_coalition) |> 
  ggplot(aes(x=pro_netanyahu_coalition, y=assertiveness_scale))+
  geom_boxplot()+
  theme_blank()+
  coord_flip()
...
responses_pilot2 %>% remove_labels() %>%
  dplyr::select(response_direction,
  sociability_scale,
                morality_scale,
                ability_scale,
                assertiveness_scale) %>%
...
pilot2 %>% 
  drop_na(civil_servant_label) %>%
#filter(civil_servant_label!="public service bureaucrat") %>% 
  ggplot(aes(x=populist_attitudes_index_4dim,y=morality_scale,color=pro_netanyahu_coalition))+
...
```{r}
responses_pilot2 %>% 
  group_by(pro_netanyahu_coalition,
  translation) %>%
    summarize(n=n()) |> 
    filter(n>=5) |> 
    arrange(pro_netanyahu_coalition,n |> desc()) |> 
    gt::gt()
```
...
```{r}
responses_pilot2 %>% 
filter(pro_netanyahu_coalition=="Netanyahu coalition",
populist_attitudes_index_4dim>=0.7) %>%
  count(response) |> 
  arrange(desc(n)) |> view()
```
...
```{r}
responses_pilot2 %>%
  ggplot(aes(x=populist_attitudes_index_4dim,y=status.x,color=pro_netanyahu_coalition))+
geom_point(alpha=0.5)+
  geom_smooth(method="lm")+
  theme_blank()

#corr test
tmod1 <- lm(sociability ~ populist_attitudes_index_4dim*pro_netanyahu_coalition, data=responses_pilot2)

summary(tmod1)

responses_pilot2 |> names()
```
...
```{r}
elite_responses <- pilot2  |> labelled::to_factor() %>% 
  dplyr::select(pro_netanyahu_coalition,
party_vote_intention,
...
write_csv(elite_responses,"elite_responses.csv")
```