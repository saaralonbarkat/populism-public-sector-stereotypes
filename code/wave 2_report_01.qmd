...
waves_combined_labs %>%
  dplyr::select(gender_label, age_group_label, religiosity_label,education_label, income_label, 
         ...
         public_sector_experience, right_left_ideology,coalition_party_vote_intention,return_wave2,completed_wave2,completed_stereotypes_wave2,duration_in_minutes,duration_in_minutes_w2,public_service_work_information
         ) %>%
  tbl_summary(
    label = list(
      gender_label ~ "Gender",
...
waves_combined_labs %>%
  dplyr::select(gender_label, age_group_label, religiosity_label,education_label, income_label, 
         sector_employment_label, public_sector_employee_current, 
         public_sector_experience, right_left_ideology,coalition_party_vote_intention,return_wave2,completed_wave2,duration_in_minutes_w2,public_service_work_information
         ) %>%
...
Returning to wave 2 and completed stereotypes (including no valence coding)
```{r}
# בחירת משתנים דמוגרפיים
waves_combined_labs %>%
  dplyr::select(gender_label, age_group_label, religiosity_label,education_label, income_label, 
...
```{r}
waves_combined_labs %>%
  dplyr::select(friendly_scale:general_image_social,civil_servant_elite_wave2,trust_civil_service_wave2,trust_civil_service
         ) %>%
 sjPlot::tab_corr(
    triangle = "lower",
 )
...
### 4 factor with 2 second order factors
...
fit.cfa.model.4_2factor_ster <- cfa(
  cfa.model.4_2factor_ster, # model formula,
  data = wave2,
  se = "robust",
  std.lv = T
)
...
waves_combined_labs %>%  labelled::to_factor() |> 
  dplyr::select(politicization_1:politicization_6,trust_civil_service_wave2,coalition_party_vote_intention
         ) %>%
...
# Direction of spontanious stereotypes


```{r}
responses_direction_wave2 %>% labelled::remove_labels() %>%
  dplyr::select(response_direction,civil_servant_label) %>%
  tbl_summary(by=civil_servant_label)
```

```{r}
responses_direction_wave2 |> 
  ggplot(aes(x = response_direction)) +
  geom_bar() +
  facet_grid(cols=vars(civil_servant_label))+
  labs(title = "Distribution of Response Direction",
       x = "Response Direction (1=Negative, 2=Neutral, 3=Positive)",
       y = "Frequency")+
  theme_minimal()
```
...
waves_combined_labs %>%
  dplyr::select(gender_label, age_group_label, religiosity_label,education_label, income_label, 
         sector_employment_label, public_sector_employee_current, 
         public_sector_experience, right_left_ideology,coalition_party_vote_intention,return_wave2,completed_wave2,completed_stereotypes_wave2,duration_in_minutes_w2,public_service_work_information
         ) %>%
